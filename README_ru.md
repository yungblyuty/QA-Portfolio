...
## [In-Game Store & Monetization Systems](https://github.com/yungblyuty/QA-Portfolio/blob/main/CASE-STUDIES/Store-System/README_ru.md)
...
В данный момент я развиваюсь в направлении QA Automation Engineering и применяю автоматизацию для решения практических QA-задач.
...
- автоматизация проверки аналитики
- повышение эффективности QA-процессов
...
Python-проект для автоматизации проверки аналитических событий мобильной игры.

Проект основан на реальных QA-сценариях из тестирования:
...
Цель проекта — показать переход от ручной проверки аналитики к автоматизированному контролю качества данных.
...
| Проверка | Цель |
|---|---|
| Проверка обязательных событий | Убедиться, что все необходимые аналитические события были отправлены |
| Проверка дублирующихся событий | Найти повторную отправку аналитических событий |
| Проверка последовательности событий | Проверить корректный порядок пользовательского сценария |
| Проверка дублирования наград | Предотвратить некорректную аналитику получения наград |
| Проверка прогрессии | Найти отсутствующие события прогрессии |

---

## Связь с ручным тестированием

Автоматизация покрывает риски, найденные во время ручного и исследовательского тестирования:

### Дублирование событий получения награды

Проблема:

При определённых сценариях аналитическое событие получения награды могло отправляться повторно.

Автоматическая проверка:

- обнаруживает дублирование событий
- предотвращает искажение аналитических данных
- защищает корректность продуктовых метрик
...
При сложных сценариях прогрессии часть аналитических событий могла отсутствовать.

Автоматическая проверка:

- проверяет наличие обязательных событий прогрессии
- выявляет отсутствующие данные
- повышает надёжность аналитики
...
[Открыть проект](https://github.com/yungblyuty/QA-Portfolio/tree/main/Automation/Analytics_Validator)
